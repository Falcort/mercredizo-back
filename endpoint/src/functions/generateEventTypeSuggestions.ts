...
export async function generateEventTypeSuggestions(request: Request, database: Knex): Promise<void> {
  ...
  const presences = await database('event_presences')
    ...
    .select('user', 'presence', 'last_name').where({ event: id })
  ...
  const likes = await database('event_type_preferences')
    .join('event_types', 'event_type_preferences.event_type', 'event_types.id')
    .join('directus_users', 'directus_users.id', 'event_type_preferences.user')
    .select('event_type', 'rating', 'event_types.name', 'directus_users.last_name')
  ...
  ratings.forEach((rating) => {
    if (myMap[rating.avg]) {
      ...
      if (current) {
        myMap[rating.avg] = [...current, { id: rating.id, name: rating.name }];
      }
    } else {
  ...
  keys.forEach((key) => {
    const shuffled: { id: string, name: string }[] = shuffle(myMap[key] || []);
