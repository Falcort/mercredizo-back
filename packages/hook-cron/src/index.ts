...
export default defineHook(({ schedule }, { database, logger }) => {
  ...
  async function findEventByDate(days = 0): Promise<Record<string, any>> {
    ...
    const event = await database('events').select('*').where({ date }).first();
    if (!event) {
  ...
  schedule('0 0 * * SUN', async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/actions/generateEvent/${date.toISODate()}`);
    } catch (e) {
  ...
  schedule('0 0 * * TUE', async (): Promise<void> => {
    ...
    try {
      ...
      await axios.patch(
        `${API_URL}/actions/${event.id}/generateEventTypeSuggestions`
      );
    } catch (e) {
