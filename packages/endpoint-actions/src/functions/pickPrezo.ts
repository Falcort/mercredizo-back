...
export default async function pickPrezo(
  request: Request,
  database: Knex
): Promise<void> {
  ...
  const presences = await database('event_presences')
    .select('user')
    .where({ event: id, presence: 'present' });
  ...
  if (potentialPrezo.length !== 0) {
    prezo =
      potentialPrezo[Math.floor(Math.random() * potentialPrezo.length)].user;
    dump.push({
